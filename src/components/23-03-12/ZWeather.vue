...
const haveData = computed(() => citys.length !== 0)
    ...
    <div class="flex">
      <div class="relative flex items-center">
        ...
        <input
          id="weather"
          ref="searchCityRef"
          v-model="searchCity" type="text" name="weather" placeholder="请输入城市"
          class="w-[250px] h-8 rounded p-2.5 text-lg"
        >
        <button
          class="clear"
          :hidden="clearInputHidden" @click="clearInput"
        >
          清空
        </button>
      </div>
      <button
        class="bg-blue-200 h-8 px-2 py-1 rounded ml-2"
        @click="getWeather"
      >
        查询
      </button>
